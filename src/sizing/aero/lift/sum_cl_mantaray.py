...
if __name__ == "__main__":
    ...
    ivc.add_output('CL_alpha_manta_eff', val=5.5 * np.ones(N), units='1/rad',
                   desc='Manta effective lift curve slope')
    

    # Ray parameters
    ivc.add_output('CL0_ray', val=0.1, desc='Ray zero-angle lift coefficient')
    ivc.add_output('CL_alpha_ray_eff', val=4.0 * np.ones(N), units='1/rad',
                   desc='Ray effective lift curve slope')

    

    # Common parameters
    ivc.add_output('alpha', val=2.0 * np.ones(N), units='deg', desc='Angle of attack')
    
    # Add subsystems to model
    prob.model.add_subsystem('inputs', ivc, promotes=['*'])
    prob.model.add_subsystem('CL', GroupCLMantaRay(N=N), promotes=['*'])
