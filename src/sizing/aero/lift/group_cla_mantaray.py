...
class GroupCLAlphaMantaRay(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('cl_alpha_ray', 
                        GroupCLAlphaRay(manta=self.options['manta'], N=N),
...
if __name__ == "__main__":
    ...
    # Manta parameters
    ivc.add_output('mach_manta', val=0.78 * np.ones(N), desc='Manta Mach number')
    ivc.add_output('phi_50_manta', val=5.0, units='deg', desc='Manta 50% chord sweep angle')

    ivc.add_output('cl_alpha_airfoil_manta', val=2*np.pi * np.ones(N), units='1/rad', desc='Manta airfoil lift curve slope')
    ...
    # Ray parameters
    ivc.add_output('mach_ray', val=0.78 * np.ones(N), desc='Ray Mach number')
    ivc.add_output('phi_50_ray', val=4.0, units='deg', desc='Ray 50% chord sweep angle')
    ivc.add_output('cl_alpha_airfoil_ray', val=2*np.pi * np.ones(N), units='1/rad', desc='Ray airfoil lift curve slope')
    ...
    prob.model.add_subsystem('cl_alpha_mantaray', GroupCLAlphaMantaRay(manta=1, ray=1, N=N), promotes=['*'])
