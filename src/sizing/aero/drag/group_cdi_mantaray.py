...
class GroupCDiMantaRay(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('manta', GroupCDiManta(ray=self.options['ray']),
                           promotes=['rho','u','mach','S_ref','d_eps_manta_d_alpha'])
        
        self.add_subsystem('ray', GroupCDiRay(manta=self.options['manta']),
                           promotes=['rho','u','mach','S_ref','d_eps_ray_d_alpha'])
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem('cdi_manta_ray', GroupCDiMantaRay(ray=1,manta=1), promotes=['*'])
