...
class GroupCDRay(om.Group):
    ...
    def setup(self):
        ...
        self.connect('sum.CD0', 'CD0_ray')
        self.connect('CDi', 'CDi_ray')
...
if __name__ == "__main__":
    ...
    ivc.add_output('d_pod', val=3.0, units='m', desc='Nacelle length')
    ivc.add_output('l_pod', val=2.0, units='m', desc='Duct outer diameter')
    ivc.add_output('Q_pod', val=1.0, desc='Duct interference factor')
    ivc.add_output('k_lam_pod', val=0.1, desc='Duct laminar flow fraction')
    ivc.add_output('num_pods', val=2, desc='Number of pods')

    ivc.add_output('S_ray', val=20, units='m**2', desc='Ray planform area')
    ivc.add_output('S_manta', val=80, units='m**2', desc='Manta planform area')



    # Add subsystems to model
    prob.model.add_subsystem('inputs', ivc, promotes=['*'])
    prob.model.add_subsystem('CD', GroupCDRay(), promotes=['*'])
    ...
    prob.model.connect('Q_pod', 'canard.Q')
    prob.model.connect('Q_ray', 'pods.Q')
