...
class FuelSystemWeight(om.ExplicitComponent):
    ...
    def setup(self):
        self.add_input("v_fuel_tot", val=1.0, units="galUS",
                      desc="total fuel quantity")
        self.add_input("v_fuel_int", val=1.0, units="galUS",
                      desc="integral tank fuel quantity")
        self.add_input("n_tank", val=1.0, units=None,
...
class TurbineWeight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        p_turbine_hp = inputs["p_turbine_unit"] / 745.7
        g = 9.806

        outputs["w_turbines"] = (inputs['n_gens'] * (71.65 + 0.3658 * p_turbine_hp)) / 2.20462 * g
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials["w_turbines", "p_turbine_unit"] = (inputs['n_gens'] * 0.3658 / 745.7) / 2.20462 * g
        partials["w_turbines", "n_gens"] = (p_turbine * 0.3658 / 745.7) / 2.20462 * g
...
class NacelleWeight(om.ExplicitComponent):
    ...
    def setup(self):
        self.add_input("num_ducts", val=1.0, units=None,
                      desc="Number of fans")
    ...
    def compute(self, inputs, outputs):
        ...
        n_gens = inputs["n_gens"]
        p_gen_unit_hp = inputs["p_gen_unit"] / 745.7
        p_turbine_unit_hp = inputs["p_turbine_unit"]  / 745.7

        #pdb.set_trace()
        g = 9.806
        outputs["w_nacelles"] = 0.14 * ( p_fan_shaft_max * num_ducts + p_gen_unit_hp * n_gens + p_turbine_unit_hp * n_gens ) / 2.20462 * g
    ...
    def compute_partials(self, inputs, partials):   
        ...
        partials["w_nacelles", "p_fan_shaft_max"] = (0.14 * num_ducts) / 2.20462 * g
        partials["w_nacelles", "num_ducts"] = (0.14 * p_fan_shaft_max) / 2.20462 * g

        partials["w_nacelles", "p_gen_unit"] = (0.14 * n_gens/ 745.7) / 2.20462 * g
        partials["w_nacelles", "n_gens"] = (0.14 * (p_gen_unit_hp + p_turbine_unit_hp)) / 2.20462 * g
...
class ElecPowertrainWeight(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials):
        ...
        for output in ["w_motors", "w_pe", "w_cbls", "w_bat"]:
...
class TurboElecPowertrainWeight(om.ExplicitComponent):
    """Computes weights of electrical powertrain components."""
...
class HybridPowertrainWeight(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem("compute_elec_ptrain_weight", TurboElecPowertrainWeight(),
...
if __name__ == "__main__":
    ...
    model.connect("n_prplsrs", ["compute_prop_weight.n_prplsrs","n_props"])
