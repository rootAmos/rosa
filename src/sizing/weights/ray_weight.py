...
from src.sizing.weights.ptrain_weight import PropulsorWeight
from src.sizing.weights.ptrain_weight import ElecPowertrainWeight
...
class WingWeight(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("w_bat", val=1.0, units="lbf", desc="Weight of battery in wings")
    ...
    def compute_partials(self, inputs, partials):   
        ...
        partials["w_wing", "s_ref"] = (0.0273*lambda_w**0.0400*q_cruise**0.0060*w_bat**0.0035*(n_ult*w_mto)**0.4900*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/(s_ref**0.2420*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "w_bat"] =(1.2600e-04*lambda_w**0.0400*q_cruise**0.0060*s_ref**0.7580*(n_ult*w_mto)**0.4900*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/(w_bat**0.9965*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "w_mto"] = (0.0176*lambda_w**0.0400*n_ult*q_cruise**0.0060*s_ref**0.7580*w_bat**0.0035*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/((n_ult*w_mto)**0.5100*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "ar_w"] = (0.0216*lambda_w**0.0400*q_cruise**0.0060*s_ref**0.7580*w_bat**0.0035*(n_ult*w_mto)**0.4900)/(np.cos(sweep_c_4_w)**2*(ar_w/np.cos(sweep_c_4_w)**2)**0.4000*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "q_cruise"] =(2.1600e-04*lambda_w**0.0400*s_ref**0.7580*w_bat**0.0035*(n_ult*w_mto)**0.4900*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/(q_cruise**0.9940*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "lambda"] = (0.0014*q_cruise**0.0060*s_ref**0.7580*w_bat**0.0035*(n_ult*w_mto)**0.4900*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/(lambda_w**0.9600*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "n_ult"] = (0.0176*lambda_w**0.0400*q_cruise**0.0060*s_ref**0.7580*w_bat**0.0035*w_mto*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/((n_ult*w_mto)**0.5100*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000)

        partials["w_wing", "t_c_w"] = -(1.0800*lambda_w**0.0400*q_cruise**0.0060*s_ref**0.7580*w_bat**0.0035*(n_ult*w_mto)**0.4900*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/(np.cos(sweep_c_4_w)*((100*t_c_w)/np.cos(sweep_c_4_w))**1.3000)

        partials["w_wing", "sweep_c_4_w"] = (0.0432*ar_w*lambda_w**0.0400*q_cruise**0.0060*s_ref**0.7580*w_bat**0.0035*np.sin(sweep_c_4_w)*(n_ult*w_mto)**0.4900)/(np.cos(sweep_c_4_w)**3*(ar_w/np.cos(sweep_c_4_w)**2)**0.4000*((100*t_c_w)/np.cos(sweep_c_4_w))**0.3000) - (1.0800*lambda_w**0.0400*q_cruise**0.0060*s_ref**0.7580*t_c_w*w_bat**0.0035*np.sin(sweep_c_4_w)*(n_ult*w_mto)**0.4900*(ar_w/np.cos(sweep_c_4_w)**2)**0.6000)/(np.cos(sweep_c_4_w)**2*((100*t_c_w)/np.cos(sweep_c_4_w))**1.3000)
...
class ElectricalSystemWeight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["w_electrical"] = 12.57 * (w_avionics)**0.51

    def compute_partials(self, inputs, partials):
        w_avionics = inputs["w_avionics"]
        
        partials["w_electrical", "w_avionics"] = 12.57 * 0.51 * (w_avionics)**-0.49
...
class FlightControlSystemWeight(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials["w_fcs", "b_w"] = (0.0197*(1.0000e-04*n_ult*w_mto)**0.8000)/b_w**0.6290

        partials["w_fcs", "n_ult"] = (4.2400e-06*b_w**0.3710*w_mto)/(1.0000e-04*n_ult*w_mto)**0.2000

        partials["w_fcs", "w_mto"] = (4.2400e-06*b_w**0.3710*n_ult)/(1.0000e-04*n_ult*w_mto)**0.2000
...
class LandingGearWeight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        w_mlg = 0.095 * (n_l * (w_mto ) ** 0.768) * (l_m ** 0.409)

        # Nose landing gear weight
        w_nlg = 0.125 * (n_l * (w_mto ) ** 0.566) * (l_n ** 0.845)
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials["w_lg", "w_mto"] = (0.0708*l_n**0.8450*n_l)/(w_mto)**0.4340 + (0.0730*l_m**0.4090*n_l)/(w_mto)**0.2320

        partials["w_lg", "l_m"] = (0.0389*n_l*(w_mto)**0.7680)/l_m**0.5910

        partials["w_lg", "l_n"] = (0.1056*n_l*(w_mto)**0.5660)/l_n**0.1550

        partials["w_lg", "n_l"] = 0.0950*l_m**0.4090*(w_mto)**0.7680 + 0.1250*l_n**0.8450*(w_mto)**0.5660
...
class RayWeight(om.Group):
    """Group containing all weight calculation components."""
    
    def setup(self):

        self.add_subsystem("prop_weight", PropulsorWeight(),promotes_inputs=[])
        self.add_subsystem("elec_ptrain_weight", ElecPowertrainWeight(), promotes_inputs=["*"], promotes_outputs=["*"])
