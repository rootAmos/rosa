...
class ComputeDuctedfan(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('thrust_unit', val= 1, desc='total thrust required', units='N')
    ...
    def compute_partials(self, inputs, partials):  
        ...
        partials['eta_prplsv', 'rho'] = (epsilon_r*thrust_unit)/(rho**2*vel*np.pi*((epsilon_r**2*vel**2)/4 + (thrust_unit*epsilon_r)/(np.pi*rho*(d_blades**2/4 - d_hub**2/4)))**(1/2)*(d_blades**2/4 - d_hub**2/4)*((vel + ((epsilon_r**2*vel**2)/4 + (thrust_unit*epsilon_r)/(np.pi*rho*(d_blades**2/4 - d_hub**2/4)))**(1/2) + vel*(epsilon_r/2 - 1))/vel + 1)**2)

        partials['eta_prplsv', 'thrust_unit'] = -epsilon_r/(rho*vel*np.pi*((epsilon_r**2*vel**2)/4 + (thrust_unit*epsilon_r)/(np.pi*rho*(d_blades**2/4 - d_hub**2/4)))**(1/2)*(d_blades**2/4 - d_hub**2/4)*((vel + ((epsilon_r**2*vel**2)/4 + (thrust_unit*epsilon_r)/(np.pi*rho*(d_blades**2/4 - d_hub**2/4)))**(1/2) + vel*(epsilon_r/2 - 1))/vel + 1)**2)
