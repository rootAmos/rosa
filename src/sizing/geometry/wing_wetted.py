...
class WettedAreaWing(om.ExplicitComponent):
    ...
    def setup(self):
        # Inputs
        self.add_input('S_exp', val=1.0, units='m**2', 
                      desc='Exposed planform area')
        self.add_input('t_c', val=1.0, 
                      desc='Thickness to chord ratio')
        self.add_input('tau', val=1.0, 
                      desc='wing tip thickness to chord ratio / wing root thickness to chord ratio')

        self.add_input('lambda', val=1.0, 
                      desc='Wing taper ratio')
        

        # Outputs
        self.add_output('S_wet', val=1.0, units='m**2', 
                       desc='Wetted area')
        
        # Declare partials
        self.declare_partials('S_wet', ['S_exp', 't_c', 'tau', 'lambda'])
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials['S_wet', 'lambda'] = 2*S_exp*((0.2500*t_c*tau)/(lambda_w+ 1) - (0.2500*t_c*(lambda_w*tau + 1))/(lambda_w+ 1)**2)
...
if __name__ == "__main__":
    ...
    ivc.add_output('lambda', val=0.3, desc='Wing taper ratio')
    
    # Add subsystems to model
    prob.model.add_subsystem('inputs', ivc, promotes=['*'])
    prob.model.add_subsystem('wing', WettedAreaWing(), promotes=['*'])
    ...
    print(f'  Taper ratio λ:        {prob.get_val("lambda")}')
    ...
    for lambda_w in lambda_range:
        prob.set_val('lambda', lambda_w)
        prob.run_model()
        S_wet_lambda_w.append(prob.get_val('S_wet')[0])
    
    plt.subplot(224)
    plt.plot(lambda_range, S_wet_lambda_w)
